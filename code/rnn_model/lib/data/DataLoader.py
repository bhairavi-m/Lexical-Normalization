...
def keyboardError(word, prox_arr):
    i = random.randint(0,len(word)-1)

    return word[:i] + random.choice(prox_arr[word[i]]) + word[i+1:]  #default is keyboard errors
...
class DataLoader(object):
                        ...
                        if(self.opt.data_augm):
                            if random.random() > (1 - self.opt.noise_ratio):
                                if iword == oword and len(iword)>1 and len(oword)>1 and not any(c.isdigit() for c in iword) and  not any(c.isdigit() for c in oword):
                                    ...
                                    # original = iword
                                    # for i in range(4):
                                    #     noisy = self.add_noise(iword)
                                    #     print(f'original {original}; noisy {noisy}')
                                    #     if (noisy != '' and noisy != ' '):
                                    #         iwordv, owordv = self.vector_repr(noisy, oword, update_mappings)
                                    #         source_vocab.add_words(iwordv)
                                    #         target_vocab.add_words(owordv)
                                    #         tweet.set_input(iwordv)
                                    #         tweet.set_output(owordv)
    ...
    def add_noise(self, word):
        ...
        if op == 2:
            return keyboardError(word, self.prox_arr)
