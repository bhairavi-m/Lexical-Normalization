...
def main():  # -logfolder -gpu 0
    opt = parser.parse_args(
        f'-eval {"-gpu 0 " if GPU else ""}-save_dir S2SChar -load_from S2SChar/model_50_char.pt -input char -data_augm -noise_ratio 0.1 -lowercase -bos -eos -batch_size 32 -share_vocab'.split(
            " "
        )
    )
    opt = change_args(opt)
    ...
    logging.basicConfig(
        filename=os.path.join(
            opt.save_dir,
            "output.log") if opt.logfolder else None,
        level=logging.INFO,
    )

    train_data, valid_data, test_data, vocab, mappings = lib.data.create_datasets(
        opt)
    ...
    for tweet in json_object:
        for input_word, output_word, target_word in zip(
            tweet["input"], tweet["output"], tweet["target"]
        ):
